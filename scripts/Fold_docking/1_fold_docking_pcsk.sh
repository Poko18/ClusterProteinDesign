...
total_mpnn=100
mpnn_per_design=5 # Filtered mpnn sequences for AF2
num_recycles=12
sampling_temp=0.2 # ProteinMPNN sampling temperature

prefix="pcsk_scaff_binder"
target_pdb="../../targets/pcsk9/inputs/pcsk9_target_short.pdb"
output="../../targets/pcsk9/outputs/fold_docking"
...
rf_out=$output/rf_dock/${SLURM_ARRAY_TASK_ID}/${prefix}_${SLURM_ARRAY_TASK_ID}
...
if [ -d "$output/rf_dock/${SLURM_ARRAY_TASK_ID}/traj" ]; then
    rm -r "$output/rf_dock/${SLURM_ARRAY_TASK_ID}/traj" # delete trajectories
fi
...
for ((i=0; i<${#input_files[@]}; i++)); do
  ...
  af_out=$output/mpnn_af2/${SLURM_ARRAY_TASK_ID}
